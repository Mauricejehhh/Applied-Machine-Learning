...
with open(dataset_pth + 'train/ids.txt') as f:
    for id in f:
        ...
        image = io.imread(dataset_pth + 'train/' + str(id) + '.jpg')
        plt.imshow(image)
        ...
        with open(dataset_pth + 'annotations_all.json') as json_data:
            ...
            for i in range(traffic_signs):
                ...
                xmin = int(bbox["xmin"] / 4) # Divide by 4 to account for resizing (2048 --> 512)
        ...
        # Cropping the image with the boundary boxes just to see what they look like and how it works
        cropped_image = resized_image[ymin:ymax, xmin:xmax]
