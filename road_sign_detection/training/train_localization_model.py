...
def collate_fn(batch: List[Tuple[torch.Tensor, Dict[str, torch.Tensor]]]
    ...
    Args:
        batch: A list of tuples containing images and their corresponding targets.

    Returns:
        A tuple of two tuples: one with image tensors, and the other with target dictionaries.
    """
...
class KFoldTrainer:
    """
    Trainer class for bounding box regression using K-Fold cross-validation.
    """

    def __init__(self, data_root: str, k_splits: int = 5, epochs: int = 5, batch_size: int = 4, lr: float = 0.001):
        """
        Initializes the KFoldTrainer with dataset path and training parameters.
    ...
    def _filter_annotations(self) -> None:
        """
        Checks and filters dataset annotations using the provided utility.
        """
        check_annotations(self.data_root)

    def train(self) -> None:
        """
        Trains the bounding box regression model using K-Fold cross-validation.
        """
        ...
        for fold_idx, (train_idx, val_idx) in enumerate(kf.split(self.dataset)):
            ...
            train_loader = DataLoader(Subset(self.dataset, train_idx), batch_size=self.batch_size,
                                      shuffle=True, collate_fn=collate_fn)
            val_loader = DataLoader(Subset(self.dataset, val_idx), batch_size=self.batch_size,
    ...
    def _plot_losses(self, train_losses_all_folds: List[List[float]], val_losses_all_folds: List[List[float]]) -> None:
        """
        Plots training and validation losses for each fold.

        Args:
            train_losses_all_folds: List of training loss lists for each fold.
            val_losses_all_folds: List of validation loss lists for each fold.
        """
    ...
    def _train_one_epoch(self, model: nn.Module, dataloader: DataLoader,
    ...
    def _validate(self, model: nn.Module, dataloader: DataLoader, loss_fn: nn.Module) -> float:
        """
    ...
    def ensemble_predict(self, image_tensor: torch.Tensor) -> torch.Tensor:
        """
        Makes a prediction by averaging outputs of all fold models.

        Args:
            image_tensor: Input image tensor of shape (1, C, H, W).

        Returns:
            Averaged bounding box prediction tensor.
        """
        predictions = []
    ...
    def save_ensemble_model(self) -> None:
        """
        Averages the weights from all fold models and saves the ensemble model.
        """
        state_dicts = []
        for fold in range(1, self.k_splits + 1):
    ...
    def visualize_sample_prediction(self, fold: int = 1, sample_idx: int = 4, use_ensemble: bool = False) -> None:
        """
        Visualizes predicted and ground truth bounding boxes for a sample image.

        Args:
            fold: Fold number to load the model from (ignored if use_ensemble=True).
            sample_idx: Index of the sample in the dataset.
            use_ensemble: Whether to use the ensemble model or a single fold.
        """
